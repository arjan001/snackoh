        ...
        <div class="page-header">
            <div class="row align-items-center">
                ...
                <div class="col-auto text-end float-end ms-auto">
                     ...
                     <?php endif; ?>
                     <!-- <button type="button" class="btn btn-info ms-2" onclick="showNotification('Test notification!', 'success')">
                         <i data-feather="bell" class="me-2"></i>Test Notification
                     </button> -->
                </div>
            </div>
        </div>
                            ...
                            <table class="table table-striped" id="permissionsTable">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <button class="btn btn-sm btn-info" onclick="viewPermissionUsage('<?php echo $permission['permission_key']; ?>')">
                                                <i data-feather="users" class="feather-16"></i> Usage
                                            </button>
                                        </td>
                                    </tr>
                                    <?php endwhile; ?>
...
<!-- Permission Usage Modal -->
<div class="modal fade" id="permissionUsageModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Permission Usage</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <div id="permissionUsageContent">
                    <!-- Content will be loaded here -->
                </div>
            </div>
        </div>
...
<div class="modal fade" id="editRolePermissionsModal" tabindex="-1">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-body" id="editRolePermissionsContent">
        <!-- Permissions editor will be loaded here -->
      </div>
    </div>
  </div>
</div>
...
function editRolePermissions(roleId) {
    $.ajax({
        ...
        data: { role_id: roleId },
        success: function(response) {
            $('#editRolePermissionsContent').html(response);
            $('#editRolePermissionsModal').modal('show');
            feather.replace();
        },
        error: function() {
            showNotification('Error loading role permissions editor', 'error');
        }
    });
}

function viewPermissionUsage(permissionKey) {
    $.ajax({
        url: 'get_permission_usage.php',
        type: 'GET',
        data: { permission_key: permissionKey },
        success: function(response) {
            $('#permissionUsageContent').html(response);
            $('#permissionUsageModal').modal('show');
        },
        error: function() {
            alert('Error loading permission usage');
        }
    });
}
